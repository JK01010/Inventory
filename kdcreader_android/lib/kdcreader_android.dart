...
class KdcreaderAndroid extends KdcreaderPlatformInterface
    with EventChannelKdcreaderAndroid {
  final MethodChannel _channel =
      const MethodChannel('plugins.flutter.koamtac/android/kdcreader');

  /// Registers this class as the default instance of [KdcreaderPlatformInterface].
  static void registerWith() {
    KdcreaderPlatformInterface.instance = KdcreaderAndroid();
  }
  ...
  Future<List<KDCDevice>?> getAvailableDeviceList(DeviceListType type,
    ...
    final devices = await _channel.invokeListMethod<Map<dynamic, dynamic>>(
        'getAvailDeviceListEx', <String, dynamic> {
      'type': listType,
    });
  ...
  Future<bool?> checkUsbPermission(KDCDevice device) {
    final deviceMap = device.toMap();
    return _channel.invokeMethod<bool>('checkUsbPermission', <String, dynamic> {
      'device': deviceMap,
    });
  }
  ...
  Future<bool?> requestUsbPermission(KDCDevice device) {
    final deviceMap = device.toMap();
    return _channel
        .invokeMethod<bool>('requestUsbPermission', <String, dynamic> {
      'device': deviceMap,
    });
  }
...
mixin EventChannelKdcreaderAndroid
    implements EventChannelKdcreaderPlatformInterface {
  final EventChannel _connectionStateEventChannel =
      EventChannel('plugins.flutter.koamtac/android/connection');

  final EventChannel _barcodeDataEventChannel =
      EventChannel('plugins.flutter.koamtac/android/barcode');
  final EventChannel _nfcDataEventChannel =
      EventChannel('plugins.flutter.koamtac/android/nfc');
  final EventChannel _msrDataEventChannel =
      EventChannel('plugins.flutter.koamtac/android/msr');
  final EventChannel _generalDataEventChannel =
      EventChannel('plugins.flutter.koamtac/android/general');

  final EventChannel _posDataEventChannel =
      EventChannel('plugins.flutter.koamtac/android/pos');

  final EventChannel _errorEventChannel =
      EventChannel('plugins.flutter.koamtac/android/error');

  final EventChannel _deviceScannedEventChannel =
      EventChannel('plugins.flutter.koamtac/android/device_scanned');
  final EventChannel _deviceScanFailedEventChannel =
      EventChannel('plugins.flutter.koamtac/android/device_scan_failed');

  final EventChannel _usbAttachedEventChannel =
      EventChannel('plugins.flutter.koamtac/android/usb_attached');
  final EventChannel _usbDetachedEventChannel =
      EventChannel('plugins.flutter.koamtac/android/usb_detached');

  // Software Decoder when separate notification is used.
  final EventChannel _swBarcodeDataEventChannel =
      EventChannel('plugins.flutter.koamtac/android/sw_barcode');
  ...
  Stream<ScanFailEvent> getDeviceScanFailedrStream() {
    return _deviceScanFailedEventChannel
        .receiveBroadcastStream()
        .map((dynamic event) => _parseScanFailedData(event));
  }

  ScanFailEvent _parseScanFailedData(dynamic event) {
    final map = event as Map<dynamic, dynamic>;
    return ScanFailEvent.fromMap(map.cast<String, dynamic>());
  }

  @override
  Stream<KDCDevice> getUsbAttachedStream() {
    return _usbAttachedEventChannel
        .receiveBroadcastStream()
        .map((dynamic event) => _parseKdcDevice(event));
  }
  ...
  Stream<KDCDevice> getUsbDetachedStream() {
    return _usbDetachedEventChannel
        .receiveBroadcastStream()
        .map((dynamic event) => _parseKdcDevice(event));
  }
