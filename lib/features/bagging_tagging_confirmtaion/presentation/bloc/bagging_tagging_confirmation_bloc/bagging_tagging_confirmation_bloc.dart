...
part 'bagging_tagging_confirmation_event.dart';
part 'bagging_tagging_confirmation_state.dart';

class BaggingTaggingConfirmationBloc
    extends
        Bloc<BaggingTaggingConfirmationEvent, BaggingTaggingConfirmationState> {
  ...
  }) : super(BaggingTaggingConfirmationState.initial()) {
    on<BaggingTaggingConfirmationEvent>((event, emit) async {
      switch (event) {
        case FetchAllConfirmationTransactionFromDb():
          await _fetchAllConfirmationTransactionFromDb(event, emit);
  ...
  Future<void> _fetchAllConfirmationTransactionFromDb(
    FetchAllConfirmationTransactionFromDb event,
    Emitter<BaggingTaggingConfirmationState> emit,
  ) async {
      ...
      (transactionList) {
        final List<GoodsReceiptsTransactionEntity> newList =
            transactionList
                .where(
                  (element) =>
                      element.isBaggingCompleted == true &&
                      element.grnId != -1 &&
