...
#import "KDCDevice.h"
#import "UHFResult.h"
#import "KDCRawResult.h"
#import "KDCLedState.h"

// Firmware Downloader
#import "KDCDownloadListener.h"

//Software Decoder
#import "KDCSWDecoderSetting.h"
#import "KDCSWDecoderActivationResult.h"
#import "KDCSWData.h"
...
- (NSMutableArray<KDCDevice *> *) GetAvailableDeviceListEx:(enum DeviceListType)type options:(NSDictionary *)options;

- (EAAccessory *) GetConnectedDevice DEPRECATED;
- (NSString *) GetConnectedDeviceName DEPRECATED;

- (KDCDevice *) GetConnectedDeviceEx;

- (NSString *) GetSDKRevisionHistory DEPRECATED;
...
- (BOOL) ConnectEx;
- (BOOL) ConnectEx:(id)targetDevice;
...
- (BOOL) IsConnected;
- (BOOL) IsConnecting;
- (BOOL) IsKDCReaderAvailable;

- (BOOL) IsBleAutoReconnectEnabled;
- (void) EnableBleAutoReconnect:(BOOL)isEnable;

- (void) SoftwareTrigger;
- (void) SoftwareTrigger:(BOOL)isWakeup;
...
- (uint8_t *) GetKDCFirmwareBuild;
- (uint8_t *) GetKDCModelName;

- (NSString *) GetKDCSerialNumber:(BOOL)fromRemote;
- (NSString *) GetKDCFirmwareVersion:(BOOL)fromRemote;
...
- (BOOL) IsMSRICSupported;

- (BOOL) IsSocialDistanceSupported;
- (BOOL) IsOCRSupported;
- (BOOL) IsMRZSupported;

- (BOOL) IsNewArchitectureModel;
- (BOOL) IsFeatureInformationExSupported;

- (BOOL) IsSLEDSupported;
...
- (void) EnableAttachAppQuantity:(BOOL)enabled;
...
- (enum EnableDisable) IsBluetoothBeepWarningEnabled;
...
- (BOOL) SetDeviceProfile:(enum DeviceProfile)profile;
- (enum DeviceProfile) GetDeviceProfile;

- (BOOL) EnableBluetoothPower:(enum EnableDisable)isEnable;
- (enum EnableDisable) IsBluetoothPowerEnabled;

//Barcode related
- (BOOL) IsAllSymbologiesEnabled DEPRECATED;
...
- (BOOL) EnableMRZ:(enum EnableDisable)enabled;
- (enum EnableDisable) IsMRZEnabled;

- (enum MRZMode) GetMRZMode;
...
// Conversion
- (BOOL) EnableDisableLASER_UPCE_AS_UPCA:(enum EnableDisable)enabled DEPRECATED;
- (enum EnableDisable) IsLASER_UPCE_AS_UPCAEnabled DEPRECATED;
- (BOOL) EnableDisableUPCE_AS_UPCA:(enum EnableDisable)enabled;
- (enum EnableDisable) IsUPCE_AS_UPCAEnabled;

- (BOOL) EnableDisableLASER_EAN8_AS_EAN13:(enum EnableDisable)enabled DEPRECATED;
- (enum EnableDisable) IsLASER_EAN8_AS_EAN13Enabled DEPRECATED;
- (BOOL) EnableDisableEAN8_AS_EAN13:(enum EnableDisable)enabled;
- (enum EnableDisable) IsEAN8_AS_EAN13Enabled;

- (BOOL) EnableDisableLASER_UPCE_AS_EAN13:(enum EnableDisable)enabled DEPRECATED;
- (enum EnableDisable) IsLASER_UPCE_AS_EAN13Enabled DEPRECATED;
- (BOOL) EnableDisableUPCE_AS_EAN13:(enum EnableDisable)enabled;
- (enum EnableDisable) IsUPCE_AS_EAN13Enabled;

- (BOOL) EnableDisableLASER_UPCA_AS_EAN13:(enum EnableDisable)enabled DEPRECATED;
- (enum EnableDisable) IsLASER_UPCA_AS_EAN13Enabled DEPRECATED;
- (BOOL) EnableDisableUPCA_AS_EAN13:(enum EnableDisable)enabled;
- (enum EnableDisable) IsUPCA_AS_EAN13Enabled;
...
- (BOOL) EnableDisableLASER_I2OF5_RETURN_CHECK_DIGIT:(enum EnableDisable)enabled;
- (enum EnableDisable) IsLASER_I2OF5_RETURN_CHECK_DIGITEnabled;
- (BOOL) EnableDisableLASER_CODE39_RETURN_CHECK_DIGIT:(enum EnableDisable)enabled;
- (enum EnableDisable) IsLASER_CODE39_RETURN_CHECK_DIGITEnabled;
...
- (enum AutoTriggerRereadDelay) GetAutoTriggerRereadDelay;
...
- (enum WedgeMode) GetDataProcessMode;
...
- (BOOL) EnableAgeVerify:(enum EnableDisable) isEnable;
- (enum EnableDisable) IsAgeVerifyEnabled;
...
- (enum DisplayFormat) GetDisplayFormat;

- (BOOL) SetAutoPowerOffTimeout:(int)powerofftimeout DEPRECATED;
- (int) GetAutoPowerOffTimeout DEPRECATED;;
...
- (enum EnableDisable) IsDisplayConnectionStatusEnabled;

- (BOOL) EnableHighBeepVolume:(enum EnableDisable)setting;
- (enum EnableDisable) IsHighBeepVolumeEnabled;
...
- (BOOL) EnableScanIfConnected:(enum EnableDisable)setting;
- (enum EnableDisable) IsScanIfConnectedEnabled;

- (BOOL) EnablePowerOnBeep:(enum EnableDisable)setting;
- (enum EnableDisable) IsPowerOnBeepEnabled;

- (BOOL) EnableBeepOnConnect:(enum EnableDisable)setting;
- (enum EnableDisable) IsBeepOnConnectEnabled;

- (BOOL) EnableMFiMode:(enum EnableDisable)setting;
- (enum EnableDisable) IsMFiEnabled;

- (enum EnableDisable) IsMFiAuthChipInstalled;

- (BOOL) SetBrightnessLevel:(enum BrightnessLevel)level;
- (enum BrightnessLevel) GetBrightnessLevel;

- (BOOL) SetLanguage:(enum Language)language;
- (enum Language) GetLanguage;

- (int) GetExtendedBatteryLevel;

- (BOOL) SetHibernationTimeout:(enum HibernationTimeout)timeout;
- (enum HibernationTimeout) GetHibernationTimeout;

- (BOOL) EnableIllumination:(enum EnableDisable)isEnable;
- (enum EnableDisable) IsIlluminationEnabled;

- (NSString *) GetModelFeatureString;

- (BOOL) EnableBatteryStatusKeyEvent:(enum EnableDisable)isEnable;
- (enum EnableDisable) IsBatteryStatusKeyEventEnabled;

// Direct
- (KDCRawResult *) SendKDCCommand:(NSData *)command timeout:(int)timeout isWakeup:(BOOL)isWakeup;
...
- (enum LEDControlMode) GetLEDControlMode;
- (BOOL) SetLEDControlMode:(enum LEDControlMode) mode;

- (BOOL) SetLEDStateEx:(NSArray <KDCLedState *>*)stateList;
...
// Scan Key Event
- (BOOL) EnableScanKeyEvent:(enum EnableDisable)isEnable;
- (enum EnableDisable) IsScanKeyEventEnabled;

// Charging Status
- (enum EnableDisable) IsKDCChargingStatus;
...
- (BOOL) EnableMSRSentinel:(enum EnableDisable)isEnable;
- (enum EnableDisable) IsMSRSentinelEnabled;
...
// HID
- (BOOL) SetHIDAutoLockTime:(enum HIDAutoLockTime)timeout;
- (enum HIDAutoLockTime) GetHIDAutoLockTime;
- (BOOL) SetHIDKeyboard:(enum HIDKeyboard)keyboard;
- (enum HIDKeyboard) GetHIDKeyboard;
- (BOOL) SetHIDInitialDelay:(enum HIDInitialDelay)delay;
- (enum HIDInitialDelay) GetHIDInitialDelay;
- (BOOL) SetHIDInterDelay:(enum HIDInterDelay)delay;
- (enum HIDInterDelay) GetHIDInterDelay;
- (BOOL) SetHIDControlCharacter:(enum HIDControlCharacter)character;
- (enum HIDControlCharacter) GetHIDControlCharacter;

// UHF
- (BOOL) IsUHFModuleAttached;
- (BOOL) EnableUHFPower:(enum EnableDisable)isEnable;
- (BOOL) IsUHFPowerEnabled;
- (BOOL) SetUHFPowerOnTime:(enum UHFPowerTime)time;
- (enum UHFPowerTime) GetUHFPowerOnTime;
- (BOOL) SetUHFPowerOffTime:(enum UHFPowerTime)time;
- (enum UHFPowerTime) GetUHFPowerOffTime;
- (BOOL) SetUHFDataFormat:(enum UHFDataFormat)format;
- (enum UHFDataFormat) GetUHFDataFormat;

- (BOOL) SetUHFPowerLevel:(enum UHFPowerLevel)level DEPRECATED;
- (enum UHFPowerLevel) GetUHFPowerLevel DEPRECATED;

- (BOOL) EnableUHFSingleRead DEPRECATED;
- (BOOL) IsUHFSingleReadEnabled DEPRECATED;
- (BOOL) EnableUHFMultipleRead DEPRECATED;
- (BOOL) IsUHFMultipleReadEnabled DEPRECATED;

- (BOOL) SetUHFReadMode:(enum UHFReadMode)mode;
- (enum UHFReadMode) GetUHFReadMode;

- (BOOL) SetUHFDataType:(enum UHFDataType)type;
- (enum UHFDataType) GetUHFDataType;

- (BOOL) IsUHFDuplicateCheckEnabled;
- (BOOL) EnableUHFDuplicateCheck:(enum EnableDisable)isEnable;
- (NSString *) GetUHFId;

- (BOOL) SetUHFReadingTimeout:(int)timeout;
- (int) GetUHFReadingTimeout;

- (BOOL) SetUHFReadTagMode:(enum UHFReadTagMode)mode;
- (enum UHFReadTagMode) GetUHFReadTagMode;

- (BOOL) IsUHFStopActiveEnabled;
- (BOOL) EnableUHFStopActiveRead:(enum EnableDisable)isEnable;

- (BOOL) SetUHFBeepCount:(int)count;
- (int) GetUHFBeepCount;

- (BOOL) IsUHFStoreDataEnabled;
- (BOOL) EnableUHFStoreData:(enum EnableDisable)isEnable;

- (BOOL) IsUHFChargingEnabled;
- (BOOL) EnableUHFCharging:(enum EnableDisable)isEnable;

- (BOOL) SetUHFReadTIDLength:(int)length;
- (int) GetUHFReadTIDLength;

- (BOOL) SetUHFReadTIDAddress:(int)address;
- (int) GetUHFReadTIDAddress;

- (BOOL) StartUHFUserReadWriteMode DEPRECATED;
- (BOOL) FinishUHFUserReadWriteMode DEPRECATED;

- (BOOL) EnableUHFBurstMode DEPRECATED;

- (BOOL) IsUHFBurstModeEnabled;
- (BOOL) EnableUHFBurstMode:(enum EnableDisable)isEnable;

- (BOOL) IsUHFKeyEventEnabled;
- (BOOL) EnableUHFKeyEvent:(enum EnableDisable)isEnable;
- (BOOL) EnableUHFKeyEventEnabled:(enum EnableDisable)isEnable DEPRECATED;

- (BOOL) SetUHFRegion:(enum UHFRegion)region;
- (enum UHFRegion) GetUHFRegion;

- (BOOL) ExecuteUHFSmartHopping;
- (BOOL) EnableUHFSmartHopping:(enum EnableDisable)isEnable;
- (BOOL) IsUHFSmartHoppingEnabled;

- (void) CancelUHFReading;

- (enum UHFModuleType) GetUHFModuleType;

- (int) GetUHFPowerLevelEx;
- (BOOL) SetUHFPowerLevelEx:(int)level;

- (int) GetUHFPowerTimeout;
- (BOOL) SetUHFPowerTimeout:(int)timeout;

- (NSString *) GetUHFModuleVersion;

- (NSArray *) GetUHFChannelList;
- (BOOL) SetUHFChannelList:(NSArray *) channelList type:(enum UHFModuleType) type region:(enum UHFRegion) region;

- (BOOL) EnableUHFAutoSessionControl:(enum EnableDisable)isEnable;
- (BOOL) IsUHFAutoSessionControlEnabled;

- (BOOL) SetUHFAutoSessionControlTimeout:(int)timeout;
- (int) GetUHFAutoSessionControlTimeout;

- (BOOL) SendUHFUniqueTag:(enum UHFUniqueType)type;
- (void) ClearUHFUniqueTag;

- (NSArray *) GetUHFTagList:(int)timeout status:(struct UHFStatus *) status;

- (BOOL) SelectUHFTag:(NSData *)epc status:(struct UHFStatus *) status;

- (NSData *) ReadUHFTagMemory:(NSData *)pwd bank:(enum UHFMemoryBank)bank start:(int)start length:(int)length status:(struct UHFStatus *) status;
- (BOOL) WriteUHFTagMemory:(NSData *)pwd bank:(enum UHFMemoryBank)bank start:(int)start length:(int)length data:(NSData *)data status:(struct UHFStatus *) status;

- (BOOL) SetUHFTagLock: (NSData *)pwd mask:(int)mask status:(struct UHFStatus *) status;

- (BOOL) KillUHFTag:(NSData *)pwd status:(struct UHFStatus *) status;

- (BOOL) SetUHFUntraceable:(int)xpc epc:(int)epc tid:(int)tid user:(int)user range:(int)range pwd:(NSData *)pwd status:(struct UHFStatus *)status;

- (BOOL) SetUHFSelectParameter:(int)target action:(int)action memBank:(UHFMemoryBank)memBank pointer:(int)pointer length:(int)length mask:(NSData *)mask isTruncated:(BOOL)isTruncated status:(struct UHFStatus *)status;
- (UHFResult *) GetUHFSelectParameter:(struct UHFStatus *)status;

- (BOOL) SetUHFQueryParameter:(int)dr cycle:(int)cycle tRext:(int)tRext sel:(int)sel session:(int)session target:(int)target slotNum:(int)slotNum status:(struct UHFStatus *)status;
- (UHFResult *)GetUHFQueryParameter:(struct UHFStatus *)status;

- (NSData *) AuthenticateUHFTag:(int)senRep incRepLen:(int)incRepLen csi:(int)csi length:(int)length message:(NSData *)message status:(struct UHFStatus *)status;


// Social Distance Finding
- (BOOL) SetSDFindingRange:(enum SDFindingRange)range;
- (enum SDFindingRange) GetSDFindingRange;

- (BOOL)EnableSDFindingHostAlert:(enum EnableDisable)isEnable;
- (enum EnableDisable)IsSDFindingHostAlertEnabled;

- (BOOL)EnableSDFindingBeep:(enum EnableDisable)isEnable;
- (enum EnableDisable)IsSDFindingBeepEnabled;

- (BOOL)EnableSDFindingVibrator:(enum EnableDisable)isEnable;
- (enum EnableDisable)IsSDFindingVibratorEnabled;

- (BOOL)EnableSDFindingStore:(enum EnableDisable)isEnable;
- (enum EnableDisable)IsSDFindingStoreEnabled;

- (BOOL)SetSDFindingSecurity:(enum SDFindingSecurity)security;
- (enum SDFindingSecurity)GetSDFindingSecurity;

- (BOOL)SetSDFindingCustomKeyData:(NSString *)hexKey;
- (NSString *)GetSDFindingCustomKeyData;

// MSRIC
- (BOOL) IsMSRICModuleAttached;
- (NSString *) SendMSRICData:(NSString *)message;
- (NSData *) SendMSRICDataWithNSData:(NSData *)message;
...
- (int) ReadCard_POS:(short)target timeout:(short)timeout;
- (int) CancelReadCard_POS:(short)target;
...
- (int) ReadEmvCLCard_POS:(short)pinblockFormat maxDigit:(short)maxDigit transType:(short)transType amount:(int)amount otherAmount:(int)otherAmount onlinePINSupport:(short)onlinePINSupport timeout:(short)timeout;
- (int) StopEmvCLTransaction_POS;
...
- (BOOL) SetConnectionMode:(enum ConnectionMode)mode;
- (enum ConnectionMode) GetConnectionMode;

// BLE
- (BOOL) StartScan;
- (BOOL) StartScan:(NSString *)data isHidIncluded:(BOOL)isHidIncluded;
- (void) StopScan;


//==============================================================================
// Firmware Download APIs
//==============================================================================
- (void) DownloadFirmware:(enum DownloadType)type data:(NSData *)data listener:(id<KDCDownloadListener>)listener;
- (BOOL) IsDownloadV2Supported;


//==============================================================================
// Software Decoder APIs
//==============================================================================
- (KDCSWDecoderActivationResult *) ActivateSoftwareDecoder:(NSString*)activationCode;

- (void) SetActivationPeriod: (NSInteger) period;

- (BOOL) IsSoftwareDecoderActivated;
- (void) SetSoftwareDecoderContext:(UIViewController *)context;

- (NSString *) GetSoftwareDecoderDeviceId;

- (BOOL) SetSoftwareDecoderSetting:(KDCSWDecoderSetting *) setting;
- (KDCSWDecoderSetting *) GetSoftwareDecoderSetting;

- (BOOL) SetSoftwareDecoderFullMatching:(NSString *) filter;
- (BOOL) SetSoftwareDecoderPartialMatching:(NSString *) filter offset:(NSInteger) offset;
- (BOOL) EnableSoftwareDecoderSendAll:(enum EnableDisable)isEnable;

- (void) EnableSoftwareDecoderNotification:(BOOL) isEnable;
- (void) EnableSoftwareDecoderBarcodeListener:(BOOL) isEnable;

- (KDCSWData *) ParseHSMDecodeResult:(HSMDecodeResult *) barcode;
- (void) FeedHSMDecodeResults:(HSMDecodeResultArray *) barcodes;

- (UIImage *) GetSoftwareDecoderLastProcessedImage;
- (UIImage *) GetSoftwareDecoderLastDecodedImage;
- (UIImage *) GetSoftwareDecoderLastBarcodeImage:(BarcodeBounds *) bounds;

- (void) ScanSoftwareDecoderBarcodeInFreezeMode;
- (void) RegisterSoftwareDecoderFreezeFrameCallback:(id<FreezeFrameListener>) listener;

- (HSMDecoder *) GetSoftwareDecoderHSM;

- (void) RegisterSoftwareDecoderPlugin:(SwiftPlugin *) plugin;
- (void) UnregisterSoftwareDecoderPlugin:(SwiftPlugin *) plugin;

- (BOOL) InitSoftwareDecoderCameraConnection;
- (BOOL) ReleaseSoftwareDecoderCameraConnection;
